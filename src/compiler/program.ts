    ...
    export function createProgram(rootNamesOrOptions: readonly string[] | CreateProgramOptions, _options?: CompilerOptions, _host?: CompilerHost, _oldProgram?: Program, _configFileParsingDiagnostics?: readonly Diagnostic[]): Program {
        ...
        function getJSSyntacticDiagnosticsForFile(sourceFile: SourceFile): DiagnosticWithLocation[] {
            return runWithCancellationToken(() => {
                ...
                function walk(node: Node, parent: Node) {
                    ...
                    switch (node.kind) {
                        ...
                        default:
                            ...
                            if (node.flags & NodeFlags.InTypeComment) {
                                if (node.kind === SyntaxKind.FunctionDeclaration && (node as FunctionDeclaration).body === undefined) {
                                    // it's an overload
                                    // TODO: make sure it still errors if there is no body
                                    return "skip";
                                }
                    ...
                    switch (parent.kind) {
                        ...
                        case SyntaxKind.VariableStatement:
                            ...
                            if (nodes === (parent as VariableStatement).modifiers) {
                                checkModifiers((parent as VariableStatement).modifiers!, parent.kind === SyntaxKind.VariableStatement, !!(parent.flags & NodeFlags.InTypeComment));
                                return "skip";
                            }
                            break;
                        case SyntaxKind.PropertyDeclaration:
                        ...
                        case SyntaxKind.Parameter:
                            ...
                            if (nodes === (parent as ParameterDeclaration).modifiers && nodes.some(node => isModifier(node) && !(node.flags & NodeFlags.InTypeComment))) {
                ...
                function checkModifiers(modifiers: NodeArray<ModifierLike>, isConstValid: boolean, inTypeComment?: boolean) {
                    for (const modifier of modifiers) {
                        ...
                        switch (kind) {
                            ...
                            case SyntaxKind.DeclareKeyword:
                                if (kind === SyntaxKind.DeclareKeyword && modifier.flags & NodeFlags.InTypeComment && !inTypeComment) {
